...
def _ensure_module_type(nb, manufacturer_name, model, profile, attribute_data=None):
    ...
    if existing:
        ...
        if hasattr(existing, "profile") and existing.profile:
            existing_profile_id = (
                existing.profile.id if hasattr(existing.profile, "id") else existing.profile
            )
        ...
        if update_needed:
            ...
            logger.info(
                "Module type '%s / %s' already correct — skipping", manufacturer_name, model
            )
        return existing
...
GPU_TYPES = [
    {
        "manufacturer": "NVIDIA",
        "model": "RTX 4000 Ada",
        "attributes": {"memory_gb": 20, "form_factor": "PCIe"},
    },
    {
        "manufacturer": "NVIDIA",
        "model": "RTX A5000",
        "attributes": {"memory_gb": 24, "form_factor": "PCIe"},
    },
    {
        "manufacturer": "NVIDIA",
        "model": "RTX 6000 Ada",
        "attributes": {"memory_gb": 48, "form_factor": "PCIe"},
    },
    {
        "manufacturer": "NVIDIA",
        "model": "RTX 4090",
    ...
    },
    {
        "manufacturer": "NVIDIA",
        "model": "H100 PCIe",
        "attributes": {"memory_gb": 80, "form_factor": "PCIe", "tdp_watts": 350},
    },
]
...
SSD_TYPES = [
    {
        ...
        "model": "D7-P5520 3.84TB",
        "attributes": {"size": 3840, "type": "NVME", "form_factor": "U.2", "interface": "Gen4"},
    },
    {
        "manufacturer": "Samsung",
        "model": "PM9A3 1.92TB",
        "attributes": {"size": 1920, "type": "NVME", "form_factor": "E1.S", "interface": "Gen4"},
    },
    {
        "manufacturer": "Samsung",
        "model": "PM9A3 3.84TB",
...
NIC_TYPES = [
    {
        "manufacturer": "Intel",
        "model": "E810-XXVDA2 25GbE",
        "attributes": {"port_count": 2, "speed_gbps": "25", "form_factor": "PCIe"},
    },
    {
        "manufacturer": "NVIDIA",
        "model": "ConnectX-6 Dx 100GbE",
        "attributes": {"port_count": 2, "speed_gbps": "100", "form_factor": "PCIe"},
    },
    {
        "manufacturer": "Broadcom",
        "model": "BCM57416 OCP 25GbE",
        "attributes": {"port_count": 2, "speed_gbps": "25", "form_factor": "OCP"},
    },
]
...
DIMM_TYPES = [
    {
        "manufacturer": "Samsung",
        "model": "M393A8G40AB2-CWE 64GB",
        "attributes": {"size": 64, "class": "DDR4", "data_rate": 3200, "ecc": True},
    },
    {
        "manufacturer": "Samsung",
        "model": "M321R8GA0BB0-CQKZJ 64GB",
        "attributes": {"size": 64, "class": "DDR5", "data_rate": 4800, "ecc": True},
    },
    {
        "manufacturer": "SK Hynix",
        "model": "HMCG94AEBRA109N 64GB",
        "attributes": {"size": 64, "class": "DDR5", "data_rate": 4800, "ecc": True},
    },
]
...
PSU_TYPES = [
    {
        "manufacturer": "Delta",
        "model": "DPS-2400AB 2400W",
        "attributes": {"wattage": 2400, "input_current": "AC", "efficiency": "80+ Platinum"},
    },
    {
        "manufacturer": "Liteon",
        "model": "PS-2162-5L 1600W",
        "attributes": {"wattage": 1600, "input_current": "AC", "efficiency": "80+ Platinum"},
    },
]
...
def run(nb):
    ...
    for spec in GPU_TYPES:
        _ensure_module_type(
            nb, spec["manufacturer"], spec["model"], gpu_profile, spec.get("attributes")
        )
    for spec in CPU_TYPES:
        _ensure_module_type(
            nb, spec["manufacturer"], spec["model"], cpu_profile, spec.get("attributes")
        )
    for spec in SSD_TYPES:
        _ensure_module_type(
            nb, spec["manufacturer"], spec["model"], disk_profile, spec.get("attributes")
        )
    for spec in NIC_TYPES:
        _ensure_module_type(
            nb, spec["manufacturer"], spec["model"], nic_profile, spec.get("attributes")
        )
    for spec in DIMM_TYPES:
        _ensure_module_type(
            nb, spec["manufacturer"], spec["model"], memory_profile, spec.get("attributes")
        )
    for spec in PSU_TYPES:
        _ensure_module_type(
            nb, spec["manufacturer"], spec["model"], psu_profile, spec.get("attributes")
        )
